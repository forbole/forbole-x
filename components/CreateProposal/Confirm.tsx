...
const ConfirmProposal: React.FC<ConfirmProposalProps> = ({ accounts }) => {
    ...
    <Box pt={4}>
        ...
        <Grid item xs={6}>
          ...
          <Box display="flex" alignItems="center">
            <Autocomplete
              ...
              filterOptions={(options: string[], { inputValue }: any) =>
                options
                  .filter((o) => typesMap[o].name.toLowerCase().includes(inputValue.toLowerCase()))
                  .slice(0, 10)
              }
      ...
      <Box display="flex" justifyContent="flex-end" pt={6}>
        <Button
          ...
          color="primary"
          disabled={network === undefined || type === undefined || title === '' || description === '' ? true : false}
          // onClick={() =>
          //   onConfirm(
          //     delegations
