    ...
    <Box display="flex">
      <PieChart width={200} height={200}>
        <Pie
          ...
          paddingAngle={3}
        >
          {voteSummary.data.map((_x: any, index: any) => (
            <Cell key={_x.value} fill={COLORS[index % COLORS.length]} cornerRadius={40} />
          ))}
        </Pie>
      </PieChart>
      <Box ml={4}>
        <Typography variant="subtitle1" className={classes.title}>
          {`${t('voted')} (${formatPercentage(voteSummary.percentage, lang)})`}
        </Typography>
